...
def parse_gps(nmea_sentence: str) -> dict:
    ...
    cleaned = nmea_sentence.strip()
    
    if not cleaned.startswith("$"):
        # Attempt to parse as raw lat, lon
        try:
            parts = cleaned.split(",")
            ...
            raise ValueError(f"Failed to parse raw GPS lat/lon: {e}")
    else:
        try:
            msg = pynmea2.parse(cleaned)
    ...
    try:
        if 'msg' in locals() and msg.timestamp:
