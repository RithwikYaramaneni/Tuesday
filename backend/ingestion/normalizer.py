"""
normalizer.py — Person A: Data Ingestion & Normalization
The single entry point for all signal parsing.

Person C calls this file ONLY — they don't need to touch
gps_parser, address_geocoder, or plus_code_converter directly.

Usage:
    from ingestion.normalizer import normalize_signals

    signals = [
        {"type": "GPS",     "data": "$GPGGA,092311,1304.96,N,08016.24,E,1,07,1.2,14.5,M,,,,"},
    ...
    location_points = normalize_signals(signals)
"""
...
def normalize_signals(signals: list) -> list:
    """
    Takes the raw signals list from the POST /locate request body.
    Returns a list of LocationPoint dicts (failed signals are skipped with a warning).
...
def _parse_one(signal_type: str, data: str) -> dict:
    """
    Routes each signal to the correct parser based on its type.
    Raises ValueError if the type is unsupported.
    """
...
def _parse_cell(data: str) -> dict:
    """
    Parses a CELL tower signal.
    Expects JSON string: {"tower_lat": ..., "tower_lon": ..., "signal_dbm": ...}
    """
    import uuid
...
def _parse_landmark(data: str) -> dict:
    """
    Handles LANDMARK signals — plain text descriptions from the caller.
    e.g. "near the red building next to a petrol station"
    spaCy NLP (Person A stretch goal) would go here.
    For now, raises an error so Person C knows it's not yet resolved.
    """
...
def _parse_wifi(data: str) -> dict:
    """
    Handles WIFI signals.
    Expects JSON string: {"lat": ..., "lon": ..., "accuracy_m": ...}
    """
    import uuid
    from datetime import datetime, timezone
