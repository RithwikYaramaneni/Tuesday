...
try:
    ...
    def normalize(signal_type: str, signal_data: str) -> LocationPoint:
        ...
        if signal_type == "GPS":
            # Try to extract lat/lon from NMEA $GPGGA sentence
            ...
            # Mock: What3Words — use a hard-coded Chennai coordinate
            return LocationPoint(
                signal_id=str(uuid.uuid4()),
                latitude=13.0830,
                longitude=80.2712,
            ...
            # Mock: geocoded address — use a hard-coded Chennai coordinate
            return LocationPoint(
                signal_id=str(uuid.uuid4()),
                latitude=13.0835,
                longitude=80.2700,
...
try:
    ...
    def fuse(signals: List[LocationPoint]) -> FusedLocation:
        """
...
def locate(request: LocateRequest):
    ...
    try:
        ...
        for signal in request.signals:
            signal_type = signal.get("type", "LANDMARK")
            signal_data = signal.get("data", "")
            point = normalize(signal_type, signal_data)
